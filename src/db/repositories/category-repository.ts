...
import { categories, categoryGroups, type CategoryRow, type CategoryGroupRow } from "../drizzle/schema";
...
// Convert database row to Category type (with word count)
const rowToCategory = (row: CategoryRow & { wordCount: number }): Category => ({
  id: row.id,
  groupId: row.groupId,
  name_en: row.nameEn,
  name_vi: row.nameVi,
...
export const getCategoryGroupById = async (id: string): Promise<CategoryGroup | undefined> => {
  const db = await getVocabDrizzle();
  const rows = await db.select().from(categoryGroups).where(eq(categoryGroups.id, id));
...
export const getCategoriesGrouped = async (): Promise<Map<CategoryGroup, Category[]>> => {
  ...
  for (const group of groups) {
    const categoriesInGroup = allCategories.filter((c) => c.groupId === group.id);
